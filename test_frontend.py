...
def test_initial_test(setup_app):
    at = setup_app
    at.run()
...
def test_movie_details_display(setup_app):
    ...
    for movie in MOCK_MOVIES:
        ...
        assert any(overview_text in text for text in markdown_texts), f"Overview for {movie['title']} not found"
        assert any(rating_text in text for text in markdown_texts), f"Rating for {movie['title']} not found"
